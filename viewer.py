...
<body>
    ...
    <script>
        ...
        const fuseItems = Object.keys(comparisons).map(key => ({ key }));
        ...
        function filterTests(query) {
            ...
            if (q) {
                visibleKeys = new Set(fuse.search(q).map(r => r.item.key));
            }
            let visible = 0;
            document.querySelectorAll('.test-item').forEach(el => {
                const match = !visibleKeys || visibleKeys.has(el.dataset.key);
